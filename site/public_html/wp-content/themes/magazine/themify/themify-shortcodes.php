...
function themify_parse_shortcode_query_atts( $atts, $shortcode = '', $defaults = array() ) {
	...
	if ( '0' !== $category ) {
		...
		$ids_in = array_filter( $category, fn($a) => is_numeric( $a ) && "-" !== $a[0] );
		$ids_not_in = array_filter( $category, fn($a) => is_numeric( $a ) && "-" === $a[0] );
		$slugs_in = array_filter( $category, fn($a) => ! is_numeric( $a ) && "-" !== $a[0] );
		$slugs_not_in = array_filter( $category, fn($a) => ! is_numeric( $a ) && "-" === $a[0] );
		...
		if ( ! empty( $slugs_not_in ) ) {
			$query_args['tax_query'][] = array(
				'taxonomy' => $taxonomy,
				'field' => 'slug',
				'terms' => array_map( fn($a) => substr( $a, 1 ), $slugs_not_in ), // remove the minus sign (first character)
				'operator' => 'NOT IN'
			);
		}
	}
